/**
 * Runaway Train — Cloudflare Worker
 * ---------------------------------
 * Serves the static site (assets binding) and handles POST /api/inquiry:
 *  1. Logs every submission to the band's Google Sheet via the Apps
 *     Script receiver (Bookings / Requests tabs).
 *  2. For bookings, emails the band via Resend from the band's domain.
 *
 * Required Worker secret: RESEND_API_KEY (Settings → Variables & Secrets).
 * Without it the site still works — submissions log to the sheet, no email.
 */
